    ...
    <div class="min-h-screen bg-frost">
          ...
          <div class="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div class="flex flex-wrap gap-3">
                <a routerLink="/vehiculos" class="btn-azure py-3 px-6">Ver vehículos disponibles</a>
                <a routerLink="/tramitacion" class="btn-outline btn-outline-azure py-3 px-6">Solicitar presupuesto</a>
              </div>
            </div>
            <!-- Stats -->
      ...
      <div class="container mx-auto max-w-5xl px-4 py-16">
        <div class="text-center mb-12">
          <p class="divider-azure justify-center mb-4">Nuestros servicios</p>
        ...
        <div class="grid md:grid-cols-3 gap-6 mb-16">
          @for (svc of services; track svc.title) {
            <div class="bg-white rounded-2xl p-7 shadow-card hover:shadow-card-hover transition-all duration-300 group">
              <div class="w-12 h-12 bg-azure/10 rounded-xl flex items-center justify-center mb-5 group-hover:bg-azure/20 transition-colors">
              ...
              <p class="text-sm leading-relaxed mb-5" style="color:rgba(10,46,77,0.6)">{{ svc.description }}</p>
              <ul class="space-y-2">
                @for (feat of svc.features; track feat) {
                  <li class="flex items-center gap-2 text-xs" style="color:rgba(10,46,77,0.7)">
                    <svg class="w-3.5 h-3.5 text-azure shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="bg-white rounded-2xl shadow-card overflow-hidden mb-16">
          <div class="px-8 py-6 border-b" style="border-color:rgba(10,46,77,0.08)">
            <h2 class="font-heading text-xl font-bold text-navy">Rutas más solicitadas</h2>
            <p class="text-sm mt-1" style="color:rgba(10,46,77,0.5)">Precios orientativos. Solicita un presupuesto exacto para tu vehículo.</p>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead>
                <tr style="background:rgba(10,46,77,0.03)">
                  <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:rgba(10,46,77,0.5)">Origen</th>
                  <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:rgba(10,46,77,0.5)">Destino</th>
                  <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:rgba(10,46,77,0.5)">Método</th>
                  <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:rgba(10,46,77,0.5)">Plazo</th>
                  <th class="text-right px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:rgba(10,46,77,0.5)">Precio desde</th>
                </tr>
              </thead>
              <tbody>
                @for (route of routes; track route.from + route.to; let odd = $odd) {
                  <tr [class]="odd ? '' : 'bg-navy/[0.02]'" style="border-bottom:1px solid rgba(10,46,77,0.06)">
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-sm" style="color:rgba(10,46,77,0.6)">{{ route.days }}</td>
                    <td class="px-6 py-4 text-right">
                      <span class="font-heading font-bold text-azure">{{ route.price }}</span>
                    </td>
                  </tr>
                }
              </tbody>
        ...
        <div class="mb-16">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @for (step of steps; track step.num) {
              <div class="relative">
                @if (!$last) {
                  <div class="hidden lg:block absolute top-5 left-full w-full h-px" style="background:rgba(34,167,210,0.3);z-index:0"></div>
                }
                <div class="relative z-10 text-center">
                  <div class="w-10 h-10 rounded-full bg-azure flex items-center justify-center text-navy font-bold font-heading mx-auto mb-4">
                    {{ step.num }}
                  </div>
                  <h3 class="font-semibold text-navy text-sm mb-2">{{ step.title }}</h3>
                  <p class="text-xs leading-relaxed" style="color:rgba(10,46,77,0.6)">{{ step.desc }}</p>
                </div>
              </div>
            }
          </div>
        ...
        <div class="bg-navy rounded-2xl p-8 lg:p-12 mb-16">
          <div class="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <p class="divider-azure mb-5">Seguimiento en tiempo real</p>
              <h2 class="font-heading text-2xl lg:text-3xl font-bold text-frost mb-4">
                Sabe dónde está tu vehículo en todo momento
              </h2>
              <p class="leading-relaxed mb-6" style="color:rgba(244,248,251,0.7)">
              ...
              <ul class="space-y-3">
                @for (item of trackingFeatures; track item) {
                  <li class="flex items-center gap-3 text-sm" style="color:rgba(244,248,251,0.8)">
                    <svg class="w-4 h-4 text-azure shrink-0" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                    </svg>
                    {{ item }}
                  </li>
              ...
              @for (event of mockTracking; track event.time) {
                <div class="flex gap-4 items-start">
                  <div class="flex flex-col items-center">
                    <div [class]="event.done ? 'w-3 h-3 rounded-full bg-azure' : 'w-3 h-3 rounded-full border-2 border-white/30'"></div>
                    @if (!$last) {
                      <div class="w-px flex-1 mt-1" style="height:28px;background:rgba(255,255,255,0.15)"></div>
                    }
                  </div>
                  <div class="pb-4">
                    <p [class]="event.done ? 'text-sm font-medium text-frost' : 'text-sm text-frost/40'">{{ event.label }}</p>
                    <p class="text-xs mt-0.5" style="color:rgba(244,248,251,0.4)">{{ event.time }}</p>
                  </div>
                </div>
              }
            </div>
        ...
        <!-- ── Insurance ───────────────────────────────────────────────── -->
        <div class="bg-azure/5 border border-azure/20 rounded-2xl p-8 mb-16">
          <div class="flex flex-col lg:flex-row gap-8 items-center">
            <div class="w-16 h-16 bg-azure/20 rounded-2xl flex items-center justify-center shrink-0">
              <svg class="w-8 h-8 text-azure" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex-1">
              ...
              <p class="text-sm leading-relaxed" style="color:rgba(10,46,77,0.7)">
            ...
            <a routerLink="/tramitacion" class="btn-azure py-3 px-6 shrink-0 whitespace-nowrap">
        ...
        <div class="text-center">
          ...
          <p class="mb-6" style="color:rgba(10,46,77,0.6)">Solicita un presupuesto personalizado sin compromiso</p>
