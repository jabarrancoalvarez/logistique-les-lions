...
| 21 | **Los tests no ven ni el binding ni el SQL** | Las pruebas son de Application y corren sobre el proveedor **en memoria**, que ni enlaza la query string ni traduce LINQ a SQL. Por eso llegaron a producción tres clases de fallo que ninguna prueba podía ver: el binding de `[AsParameters]` con tipos no anulables (400 con cuerpo vacío), la cultura `de-DE` inexistente en la imagen alpine (500 en ofertas, alertas, recordatorios y PDF) y un `Distinct()` sobre un `record` que PostgreSQL no traduce (500 en el journal). Haría falta `WebApplicationFactory` y, para lo segundo, PostgreSQL en Docker | Detectado probando en producción |
